...
interface UnauthorizedContractAccessProps {
  ...
  export default async function UnauthorizedContractAccess({ contractToken }: UnauthorizedContractAccessProps) {
    const contract: NftContract = await getContractMetadata(contractToken);
      return <p>You are unauthorized to see this. You must own a {contract.name} NFT</p>;
  }